...
public class Camera2Source {
    ...
    public static class Builder {
        private final FaceDetectionProcessor mDetector;
    ...
    private class FrameProcessingRunnable implements Runnable {
        private FaceDetectionProcessor mDetector;
        ...
        FrameProcessingRunnable(FaceDetectionProcessor detector) {
            mDetector = detector;
        }
            ...
            while (true) {
                synchronized (mLock) {
                    ...
                    FirebaseVisionImageMetadata metadata = new
                            ...
                            .setWidth( mPreviewSize.getWidth()/4)
                            .setHeight(mPreviewSize.getHeight()/4)
                            .setFormat(ImageFormat.NV21)
                            .setRotation(getDetectorOrientation(mSensorOrientation))
                            .build();
                            outputFrame = FirebaseVisionImage.fromByteBuffer(ByteBuffer.wrap(quarterNV21(mPendingFrameData, mPreviewSize.getWidth(), mPreviewSize.getHeight())),metadata);
